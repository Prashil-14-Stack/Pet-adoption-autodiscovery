...
awsDiscovery() {
    \$AWSBIN ec2 describe-instances --filters Name=tag:aws:autoscaling:groupName,Values=docker-stage \\
...
instanceUpdate() {
  sleep 30
  ansible-playbook -i /etc/ansible/stage-hosts /etc/ansible/stage-env-playbook.yml --extra-vars "ansible_python_interpreter=/usr/bin/python3.9"
  sleep 30
} 
