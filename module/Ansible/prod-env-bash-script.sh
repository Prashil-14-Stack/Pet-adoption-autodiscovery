...
awsDiscovery() {
    \$AWSBIN ec2 describe-instances --filters Name=tag:aws:autoscaling:groupName,Values=docker-prod \\
...
instanceUpdate() {
  sleep 30  
  ansible-playbook -i /etc/ansible/prod-hosts /etc/ansible/prod-env-playbook.yml --extra-vars "ansible_python_interpreter=/usr/bin/python3.9" 
  sleep 30
} 
