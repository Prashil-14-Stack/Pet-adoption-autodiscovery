locals{
    sonarqube-user-data=<<-EOF
#!/bin/bash

sudo apt update -y

#***Firstly Modify OS Level values***
...
# "**Enable and start, so it starts when system boots up***"
...
sudo bash -c 'echo "
sonar.jbdc.username=sonar
sonar.jbdc.password=Admin123
sonar.jdbc.url-jdbc:postgresql://localhost/sonarqube
sonar.search.javaOpts=-Xmx512m -Xms512m- XX:+HeapDumpOnOutOfMemoryError" >> /opt/sonarqube/conf/sonar.propertles'
#Configure such that SonarQube starts on boot up
sudo touch /etc/systemd/system/sonarqube.service
#Configuring so that we can run commands to start, stop al reload sonarqube service 
sudo bash -c 'echo "
[Unit]
